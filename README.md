...
我在体育理论考试里从来没遇到过单选题，因此不能测试单选题的自动勾选功能。对于单选题，本脚本只能做匹配、显示答案，并不能自动勾选。

如果你恰好遇到了单选题，请发一份包括所有单选题、单选框的HTML给我，这份HTML可以协助我增加单选题自动勾选功能。

HTML获得方法：在Chrome控制台中按<kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>C</kbd>，然后鼠标移到网页上选取元素。寻找一个包含所有单选题的元素（或整个页面），单击。控制台会自动跳到Elements页面，按F2，会出现HTML框，把框里的内容全部复制即可。

请在[这个链接](https://github.com/KevinWang15/fdty/issues)通过New issue把HTML发给我。